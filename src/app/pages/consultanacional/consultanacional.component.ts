...
@Component({
  ...
  selector: "app-consultanacional",
  templateUrl: "./consultanacional.component.html",
  styleUrls: ["./consultanacional.component.scss"]
})
@Pipe({
  name: "ConsultNacionalComponent"
})
export class ConsultNacionalComponent extends BaseConsult implements OnInit {
  @ViewChild("InputZona")
  ...
  @ViewChild("GridConsulNacional")
  GridConsulNacional: SGridComponent;
  ...
  RequestFilterData(filter: any, pageCurrent: number, pageSize: number) {
    ...
    this.GetConsultFilterLiderNacional(filter).then(
      (response: IResponseService) => {
        ...
        if (response.status == ResponseEnum.OK) {
          ...
          this.GridConsulNacional.SetNumberPages(data.numberPages);
          this.GridConsulNacional.LoadItems(data.items);
        }
      }
    );
  }
  ...
  ExportExcelLiderNacional() {
    ...
    this.GetFilterDataLiderNacionalExport(filters).then((response: IResponseService) => {
        ...
        if (response.status == ResponseEnum.OK) {
          ...
          for (let rowIndex = 0; rowIndex < data.length; rowIndex++) {
            ...
            for (let index = 0; index < columns.length; index++) {
              ...
              row.push(
                this.GridConsulNacional._FormatterValue(value, col, rowIndex)
              );
            }
            dataexport.push(row);
          }

          this.GridConsulNacional.ExportFileExcelData(dataexport);
        }
      });
    });
  }

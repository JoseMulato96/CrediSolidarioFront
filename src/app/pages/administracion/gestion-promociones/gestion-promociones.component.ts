...
export class GestionPromocionesComponent implements OnInit {
  ...
  getMenu(codMenuPrimerNivel, codMenu) {
    const subMenu = this.auth.getUser().menus
                  .filter(x => !!x.subMenus.find(t => t.appObject.code === 'MM_ADMIN_PROMOCIONES'))[0].subMenus
                  .find(x => x.appObject.code === 'MM_ADMIN_PROMOCIONES');
    ...
    submenus.titulo = 'Gestión de promociones';

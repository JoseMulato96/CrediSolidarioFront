...
import { ListarConceptosFacturacionConfig } from './listar-conceptos-facturacion.config';
import { Router } from '@angular/router';
import { UrlRoute } from '@shared/static/urls/url-route';
import { BackFacadeService, FrontFacadeService } from '@core/services';

@Component({
  selector: 'app-listar-conceptos-facturacion',
  templateUrl: './listar-conceptos-facturacion.component.html',
})
export class ListarConceptosFacturacionComponent implements OnInit {

  public linkCrear: string;
  estado = true;

  configuracion: ListarConceptosFacturacionConfig = new ListarConceptosFacturacionConfig();
  filtrosGrid: any;
  ...
  ngOnInit() {
    this.linkCrear = UrlRoute.ADMINISTRACION_PROTECCIONES_ENFERMEDADES_GRAVES_NUEVO;
  }
  ...
  obtener(pagina = 0, tamanio = 10, sort = 'fechaCreacion,desc', estado = true) {
    ...
    this.backService.conceptoFacturacion.getConceptosFacturacion(params)
      .subscribe((resp: any) => {
  ...
  _alEditar(event: any) {
    this.router.navigate([
      ...
      UrlRoute.ADMINISTRACION_PROTECCIONES_GESTION_COBERTURA_CONCEPTOS_FACTURACION,
      event.concepto
    ]);
  }

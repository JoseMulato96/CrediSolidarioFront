...
import { ListarEnfermedadesGravesConfig } from './listar-enfermedades-graves.config';
...
@Component({
  selector: 'app-listar-enfermedades-graves',
  templateUrl: './listar-enfermedades-graves.component.html',
})
export class ListarEnfermedadesGravesComponent implements OnInit {

  public linkCrear: string;
  estado = true;

  configuracion: ListarEnfermedadesGravesConfig = new ListarEnfermedadesGravesConfig();
  filtrosGrid: any;
  ...
  obtener(pagina = 0, tamanio = 10, sort = 'fechaCreacion,desc', estado = true) {
    ...
    this.backService.enfermedadGrave.getEnfermedadGraves(params)
      .subscribe((resp: any) => {
  ...
  eliminar(codigoPlan: string) {
    this.backService.enfermedadGrave.deleteEnfermedadGrave(codigoPlan).subscribe(resp => {
